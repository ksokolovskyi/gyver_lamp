...
void main() {
  group('OceanFramesGenerator', () {
    final settingsVariant = GeneratorSettingsVariant.all();

    test('has correct blur value', () {
      expect(OceanFramesGenerator(dimension: 16).blur, equals(20));
    });

    test('reset works correctly', () {
      const speed = 8;
      const scale = 33;
      final generator = OceanFramesGenerator(dimension: 16);
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        final generator = OceanFramesGenerator(dimension: settings.dimension);
        ...
        await expectLater(
          ...
          matchesGoldenFile(
            'goldens/ocean_frames_generator.${settings.id}.png',
          ),
        );
      },
