...
void main() {
  group('ZebraFramesGenerator', () {
    final settingsVariant = GeneratorSettingsVariant.all();

    test('has correct blur value', () {
      expect(ZebraFramesGenerator(dimension: 16).blur, equals(20));
    });

    test('reset works correctly', () {
      const speed = 8;
      const scale = 33;
      final generator = ZebraFramesGenerator(dimension: 16);
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        final generator = ZebraFramesGenerator(dimension: settings.dimension);
        ...
        await expectLater(
          ...
          matchesGoldenFile(
            'goldens/zebra_frames_generator.${settings.id}.png',
          ),
        );
      },
