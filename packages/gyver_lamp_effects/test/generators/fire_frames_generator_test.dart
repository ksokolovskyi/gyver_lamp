...
void main() {
  group('FireFramesGenerator', () {
    final settingsVariant = GeneratorSettingsVariant.all();

    test('has correct blur value', () {
      expect(FireFramesGenerator(dimension: 16).blur, equals(5));
    });

    test('reset works correctly', () {
      const speed = 8;
      const scale = 33;
      final generator = FireFramesGenerator(
        dimension: 16,
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await expectLater(
          ...
          matchesGoldenFile('goldens/fire_frames_generator.${settings.id}.png'),
        );
      },
