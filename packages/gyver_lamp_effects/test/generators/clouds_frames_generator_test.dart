...
void main() {
  group('CloudsFramesGenerator', () {
    final settingsVariant = GeneratorSettingsVariant.all();

    test('has correct blur value', () {
      expect(CloudsFramesGenerator(dimension: 16).blur, equals(20));
    });

    test('reset works correctly', () {
      const speed = 8;
      const scale = 33;
      final generator = CloudsFramesGenerator(dimension: 16);
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        final generator = CloudsFramesGenerator(dimension: settings.dimension);
        ...
        await expectLater(
          ...
          matchesGoldenFile(
            'goldens/clouds_frames_generator.${settings.id}.png',
          ),
        );
      },
