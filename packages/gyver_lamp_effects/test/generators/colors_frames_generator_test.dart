...
void main() {
  group('ColorsFramesGenerator', () {
    final settingsVariant = GeneratorSettingsVariant.all();

    test('has correct blur value', () {
      expect(ColorsFramesGenerator(dimension: 16).blur, equals(0));
    });

    test('reset works correctly', () {
      const speed = 8;
      const scale = 33;
      final generator = ColorsFramesGenerator(dimension: 16);
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        final generator = ColorsFramesGenerator(dimension: settings.dimension);
        ...
        await expectLater(
          ...
          matchesGoldenFile(
            'goldens/colors_frames_generator.${settings.id}.png',
          ),
        );
      },
