...
void main() {
  group('PlasmaFramesGenerator', () {
    final settingsVariant = GeneratorSettingsVariant.all();

    test('has correct blur value', () {
      expect(PlasmaFramesGenerator(dimension: 16).blur, equals(20));
    });

    test('reset works correctly', () {
      const speed = 8;
      const scale = 33;
      final generator = PlasmaFramesGenerator(dimension: 16);
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        final generator = PlasmaFramesGenerator(dimension: settings.dimension);
        ...
        await expectLater(
          ...
          matchesGoldenFile(
            'goldens/plasma_frames_generator.${settings.id}.png',
          ),
        );
      },
