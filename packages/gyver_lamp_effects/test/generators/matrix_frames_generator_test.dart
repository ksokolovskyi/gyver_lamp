...
void main() {
  group('MatrixFramesGenerator', () {
    final settingsVariant = GeneratorSettingsVariant.all();

    test('has correct blur value', () {
      expect(MatrixFramesGenerator(dimension: 16).blur, equals(3));
    });

    test('reset works correctly', () {
      const speed = 8;
      const scale = 33;
      final generator = MatrixFramesGenerator(
        dimension: 16,
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await expectLater(
          ...
          matchesGoldenFile(
            'goldens/matrix_frames_generator.${settings.id}.png',
          ),
        );
      },
