...
void main() {
  group('SparklesFramesGenerator', () {
    final settingsVariant = GeneratorSettingsVariant.all();

    test('has correct blur value', () {
      expect(SparklesFramesGenerator(dimension: 16).blur, equals(3));
    });

    test('reset works correctly', () {
      const speed = 8;
      const scale = 33;
      final generator = SparklesFramesGenerator(
        dimension: 16,
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await expectLater(
          ...
          matchesGoldenFile(
            'goldens/sparkles_frames_generator.${settings.id}.png',
          ),
        );
      },
